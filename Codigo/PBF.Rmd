...
* `pc_pbf_pop_year`: porcentaje de población del PBF en el municipio.
* `acomp_freq_year`: tasa de seguimiento de la condicionalidad de la asistencia escolar (6 a 15 años).
* `ab_ef_year`: Tasa de deserción en la escuela primaria - 0 a 100.
* `ap_ef_year` : Tasa de aprobación en la escuela primaria.
* `cobalvoPBF_year` : Porcentaje de cobertura del público objetivo del PBF Fundamental.
* `porteIBGE_year` : Tamaño del municipio - población.
* `RDPC` : Renta per cápita según Censo 2010.
* `infraescolar_EM` : Indicador de infraestructura de las escuelas de EM en el municipio - continuo.
* `propextpobre` : Proporción de la población extremadamente pobre.
* `rep_ef_year` : tasa de fracaso en la escuela primaria.
* `rep_em_year` : tasa de fracaso en la escuela primaria.
* `tesouro_year` : Transferencias totales del tesoro nacional.
...
* `PIBcapita_year` : PIB municipal por capita.
* `PIB_year` : PIB municipal.
...
interest_vars <- c('pc_pbf_pop_year','acomp_freq_year','ab_ef_year','ap_ef_year','cobalvoPBF_year','porteIBGE_year','RDPC', 'infraescolar_EM', 'propextpobre','rep_ef_year','rep_em_year','tesouro_year','est_fam_PBF_2006','est_fam_PBF_2010','year','agua_esgoto_inadeq_2010','pc_lixo_2010','pc_luz_2010','PIBcapita_year','PIB_year','IDHM_E_2010','IDHM_L_2010','IDHM_R_2010')

data <- data[,interest_vars]
```
