...
stargazer(model1, model2, model3, type = "html", title = "Efectos de Bolsa Familia sobre Tasa de Desercion Escolar, PIB per capita e IDH",out = "models_summary.html")

htmltools::includeHTML("models_summary.html")
```

En cada resumen, el coeficiente de la variable treatment es de interés primordial. Este coeficiente representa el cambio promedio en la variable de resultado correspondiente por cada incremento unitario en la variable de tratamiento, controlando por las otras variables en el modelo.
...
En última instancia, estos resultados pueden sugerir fallas en la implementación, ya sea por negligencias administrativas en todo el sistema, e incluso conductas más turbias.  Una línea de investigación que puede ser relevante explorar en futuros estudios involucra la posible interferencia de la corrupción en la implementación y eficacia de la política.
